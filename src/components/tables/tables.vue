<template>
  <div>
    <div class="search-con search-con-top" v-if="searchable && searchPlace === 'top'">
      <Select class="search-col" v-model="searchKey">
        <Option :key="`search-col-${item.key}`" :value="item.key" v-for="item in columns" v-if="item.key !== 'handle'">
          {{ item.title }}
        </Option>
      </Select>
      <Input @on-change="handleClear" class="search-input" clearable placeholder="输入关键字搜索" v-model="searchValue"/>
