<template>
  <Row>
    <i-col span="16">
      <Row :key="`custom-icon-row-${i}`" v-for="i in (customIconList.length / 3)">
        <i-col :key="`custom-icon-${item}`" span="8" v-for="item in customIconList.slice((i - 1) * 3, i * 3)">
    ...
    <i-col span="8">
      <Card>
        <p class="intro-p">
          <Icon :size="10" style="margin-right: 10px;" type="heart"/>
