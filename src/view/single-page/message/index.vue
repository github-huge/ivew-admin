  ...
  <Card shadow>
    <div>
      <div class="message-page-con message-category-con">
        <Menu @on-select="handleSelect" active-name="unread" width="auto">
          <MenuItem name="unread">
            <span class="category-title">未读消息</span><Badge :count="messageUnreadCount" style="margin-left: 10px"></Badge>
          </MenuItem>
          <MenuItem name="readed">
            <span class="category-title">已读消息</span><Badge :count="messageReadedCount" class-name="gray-dadge" style="margin-left: 10px"></Badge>
          </MenuItem>
          <MenuItem name="trash">
            <span class="category-title">回收站</span><Badge :count="messageTrashCount" class-name="gray-dadge" style="margin-left: 10px"></Badge>
          </MenuItem>
        </Menu>
      </div>
      <div class="message-page-con message-list-con">
        <Spin fix size="large" v-if="listLoading"></Spin>
        <Menu
          :class="titleClass"
          ...
          width="auto"
        >
          <MenuItem :key="`msg_${item.msg_id}`" :name="item.msg_id" v-for="item in messageList">
            <div>
              <p class="msg-title">{{ item.title }}</p>
              <Badge :text="item.create_time" status="default" />
              <Button
                ...
                type="text"
                v-show="currentMessageType !== 'unread'"></Button>
            </div>
          </MenuItem>
        </Menu>
      </div>
      <div class="message-page-con message-view-con">
        <Spin fix size="large" v-if="contentLoading"></Spin>
